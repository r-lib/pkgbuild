scan_path_for_rtools <- function(debug = FALSE,
  ...
  if (gcc49) {
    ...
    gcc_path <- find_gcc49(cc_path, debug = debug)
    if (nzchar(gcc_path)) {
    ...
    if (debug)
      cat("Trying default location\n")
    gcc_path <- find_gcc49(
      file.path(install_path, paste0("mingw_", arch), "bin", "gcc.exe"),
      debug = debug
    )
    if (!nzchar(gcc_path)) {
      return(NULL)
...
find_gcc49 <- function(path, debug = FALSE) {
  if (!file.exists(path)) {
